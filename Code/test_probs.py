...
def parseArgs(argv):
  ...
  parser.add_argument('--checkpoint',type=str,default='models/model_84.pt',help='Model best check point to load and test')
  parser.add_argument('--train_loss_log', type=argparse.FileType('r',encoding='utf-8'),default='train_loss_v1.log',help='Training loss log file for plotting')
  parser.add_argument('--dev_loss_log', type=argparse.FileType('r',encoding='utf-8'),default='dev_loss_v1.log',help='Development loss log file for plotting')
  parser.add_argument('--true_pred_labels',type=str,default='true_pred_labels.txt',help="File to store true and pred labels")
  parser.add_argument('--probs',type=str,default='probs.txt',help="File to store probs labels")
...
def find_accuracy(loader,model,criterion):
  ...
  return total_loss*1.0/len(loader),accuracy,true_labels,predicted_labels,pred_prob
def main(argv):
  args=parseArgs(argv)
  ########### Data Objects #####################
  ...
  loss,accuracy,true_labels,pred_labels,pred_prob=find_accuracy(test_loader,model,criterion)
  print("Test",loss,accuracy)  
  with open(os.path.join('Results',args.true_pred_labels),'w') as fw:
   for l1,l2,l3 in zip(true_labels,pred_labels,pred_prob):
    l=[float(value) for value in l3]
    fw.write(str(l1)+" "+str(l2)+"\n")
 
  with open(os.path.join('Results',args.probs), 'w') as fw1:
   for l1,l2,l3 in zip(true_labels,pred_labels,pred_prob):
    l=[float(value) for value in l3]
    fw1.write(str(l)+"\n")
